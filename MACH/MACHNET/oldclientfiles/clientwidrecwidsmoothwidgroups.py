...
# def send_command(command, target00"):
#     """
...
class App:
    ...
    def refresh_loop(self):
        ...
        sensor_blocks = [block for block in self.blocks if block.type == "sensor"]
        if sensor_blocks:
            # Submit all sensor refresh commands concurrently.
            future_to_block = {
                self.executor.submit(send_command, "r" + block.get_command()): block
                for block in sensor_blocks
            }
            # Wait up to 0.2 seconds for the futures to complete.
            done, not_done = concurrent.futures.wait(future_to_block.keys(), timeout=0.2)
            for future in done:
                ...
                try:
                    ...
                    print(f"Error refreshing sensor for block {block.port_entry.get()}: {e}")
            # Cancel any futures that didn't complete within the timeout.
