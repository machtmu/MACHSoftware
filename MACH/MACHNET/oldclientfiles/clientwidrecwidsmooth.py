...
class App:
    def __init__(self, root):
        ...
        # Create a persistent thread pool for refresh tasks.
    ...
    def toggle_refresh(self):
        """Toggle a continuous refresh loop that sends the full configuration read command (rs...) every 100 ms."""
    ...
    def refresh_loop(self):
        """Continuously send the read command (rs...) to sensors every 100 ms using a persistent thread pool."""
        if not self.refresh_running:
            return
        sensor_blocks = [block for block in self.blocks if block.type == "sensor"]
        futures = []
        for block in sensor_blocks:
            cmd = "r" + block.get_command()
            futures.append(self.executor.submit(send_command, cmd))
        # Process responses as they come in.
        for future, block in zip(futures, sensor_blocks):
            try:
    ...
    def toggle_record(self):
        """Toggle recording: send 'Rec' when starting, and 'End' when stopping."""
