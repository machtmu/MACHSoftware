...
def send_command(command, target="192.168.1.51:9000"):
    """
...
class App:
    def __init__(self, root):
        ...
        self.refresh_after_id = None  # Used to cancel refresh callbacks

        # Top Menu Bar with commands.
        self.menu_bar = tk.Menu(root)
        self.menu_bar.add_command(label="Add Valve", command=self.add_valve)
        self.menu_bar.add_command(label="Add Sensor", command=self.add_sensor)
        self.menu_bar.add_command(label="Add Group", command=self.add_group)
        self.menu_bar.add_command(label="Extend Page", command=self.extend_page)
        self.menu_bar.add_command(label="Delete Selected", command=self.delete_selected_by_context_menu)
        self.menu_bar.add_command(label="Save", command=self.save_state)
        self.menu_bar.add_command(label="Reset", command=self.reset_state)
        root.config(menu=self.menu_bar)

        # Control panel with Groups drop down and other buttons.
        ctrl = tk.Frame(root)
        ctrl.pack(side="top", fill="x", padx=5, pady=5)
        self.group_var = tk.StringVar()
        self.group_var.set("Groups")
        self.groups_optionmenu = tk.OptionMenu(ctrl, self.group_var, ())
        self.groups_optionmenu.pack(side="left", padx=2)
        ...
        tk.Button(ctrl, text="All Valves Off", command=self.all_valves_off).pack(side="left", padx=2)
    ...
    # Modified add_sensor to accept sensor configuration parameters.
    def add_sensor(self, port="", name="", value=0.0, x=200, y=50,
                   min_val=0.0, max_val=100.0, min_voltage=0.0, max_voltage=5.0):
        block = SensorBlock(self.canvas, x, y, self, port, name, value, min_val, max_val, min_voltage, max_voltage)
        self.blocks.append(block)
        self.update_canvas_bounds()
    ...
    def update_groups_menu(self):
        menu = self.groups_optionmenu["menu"]
        menu.delete(0, "end")
        for group in self.groups:
            menu.add_command(label=group.name, command=lambda g=group: self.on_group_select(g))

    def on_group_select(self, group):
        self.group_var.set(group.name)
        self.canvas.yview_moveto(group.y / float(self.canvas_height))
        ...
        try:
            ...
            for item in blocks_data:
                ...
                if item.get("type") == "valve":
                    ...
                    # Pass sensor configuration values to add_sensor so they are restored.
    ...
    def all_valves_off(self):
        responses = []
        for block in self.blocks:
            if getattr(block, "type", None) == "valve":
                port = block.port_entry.get().strip()
                try:
                    voltage = float(block.input_voltage_entry.get().strip())
                except ValueError:
                    voltage = 0.0
                voltage_str = format_value(voltage, BUFFERVALVE + 1)
                cmd = f"w{port}0{voltage_str}"
                response = send_command(cmd)
                print(f"All Valves Off - Command sent: {cmd}\nResponse: {response}")
                responses.append(f"Valve {port}: {response}")
                block.state = False
                block.toggle_button.config(text="Off", bg="red")
        if responses:
            messagebox.showinfo("All Valves Off", "\n".join(responses))
        else:
            messagebox.showinfo("All Valves Off", "No valve commands sent.")
    ...
    def refresh_loop(self):
        ...
        self.refresh_after_id = self.root.after(100, self.refresh_loop)
