...
import json, os, grpc, concurrent.futures, time
import relay_pb2
...
class App:
    def __init__(self, root):
        ...
        # New local recording variables:
        self.local_recording = False
        self.local_record_rate = 1.0   # recordings per second (adjust as needed)
        self.local_record_after_id = None
        ...
        # New button for local recording to file:
        self.local_record_button = tk.Button(ctrl, text="Rec Here", command=self.toggle_local_recording)
        self.local_record_button.pack(side="left", padx=2)
    ...
    # New methods for local recording of valve data to CSV
    def toggle_local_recording(self):
        if not self.local_recording:
            self.local_recording = True
            self.local_record_button.config(text="Stop Rec Here")
            # Create CSV file with header if it doesn't exist
            if not os.path.exists("valve_recordings.csv"):
                with open("valve_recordings.csv", "w") as f:
                    f.write("timestamp,port,name,state,input_voltage\n")
            self.record_valve_data()
        else:
            self.local_recording = False
            self.local_record_button.config(text="Rec Here")
            if self.local_record_after_id:
                self.root.after_cancel(self.local_record_after_id)
                self.local_record_after_id = None

    def record_valve_data(self):
        if not self.local_recording:
            return
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        lines = []
        for block in self.blocks:
            if hasattr(block, "type") and block.type == "valve":
                data = block.get_data()
                # Record timestamp, port, name, state, and input_voltage
                line = f"{timestamp},{data.get('port','')},{data.get('name','')},{data.get('state','')},{data.get('input_voltage','')}\n"
                lines.append(line)
        if lines:
            with open("valve_recordings.csv", "a") as f:
                f.writelines(lines)
        # Schedule next recording based on local_record_rate (in ms)
        delay = int(1000 / self.local_record_rate)
        self.local_record_after_id = self.root.after(delay, self.record_valve_data)
